            ...
            toolbox: {
                feature: {
                    ...
                    myTool2: {
                        ...
                        onclick: function () {
                            setOptions(
                                getOptions([...new Set(data.filter(({ sentiment }) => sentiment === "positive").map(({ topic_modeling }) => topic_modeling).flat())], "🙂")
                            )
                        }
                    },
                    myTool3: {
                        ...
                        onclick: function () {
                            setOptions(
                                getOptions([...new Set(data.filter(({ sentiment }) => sentiment === "negative").map(({ topic_modeling }) => topic_modeling).flat())], "🙁")
                            )
                        }
                    },
                    saveAsImage: {},
            ...
            series: [
                {
                    ...
                    data: array.map(topic => ({ name: topic, value: 0, textStyle: { color: "#616161" } }))
                }
            ],
        }
    }
